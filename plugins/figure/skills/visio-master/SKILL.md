...
`visio-master` rebuilds diagram images into editable Visio deliverables.
...
Use the same disciplined front-half analysis expected in high-fidelity reconstruction work:
- source collection
- style extraction
- layout discipline
- image understanding
...
Do **not** use a raw `SVG -> PPTX` path for Visio output.
...
If the diagram is mostly boxes, arrows, labels, and containers, stay in `visio-master`.

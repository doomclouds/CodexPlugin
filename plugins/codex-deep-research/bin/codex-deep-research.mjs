...
// runner/src/commands/cancel.ts
import { writeFile as writeFile2 } from "node:fs/promises";
import { join as join2 } from "node:path";

// runner/src/runtime/run-store.ts
import { access, mkdir as mkdir2, readFile as readFile2, writeFile } from "node:fs/promises";
import { join } from "node:path";

// runner/src/runtime/jsonl.ts
...
// runner/src/runtime/run-store.ts
var RUN_ID_PATTERN = /^dr_\d{8}T\d{9}Z(?:_\d{2})?$/;
...
var RunStore = class {
  ...
  async writeCompletionStatus(status) {
    if (status.state !== "completed") {
      throw new Error(`Completion status must be completed, got ${status.state}`);
    }
    await this.readManifest(status.runId);
    const current = await this.readStatus(status.runId);
    if (current.state === "cancelled" || current.state !== "completed" && await this.isCancellationRequested(status.runId)) {
      if (current.state !== "cancelled") {
  ...
  async emit(runId, event) {
    ...
    await this.readManifest(runId).catch((error) => {
  ...
  async getRunDir(runId) {
    await this.readManifest(runId);
    return this.resolveRunDir(runId);
  }
  async isCancellationRequested(runId) {
    await this.readManifest(runId);
    return await access(join(this.resolveRunDir(runId), "cancel.requested")).then(() => true).catch((error) => {
      if (error.code === "ENOENT") {
        return false;
      }
      throw error;
...
// runner/src/commands/workspace.ts
function resolveWorkspaceRoot() {
  return process.env.INIT_CWD ?? process.cwd();
}

// runner/src/commands/cancel.ts
var TERMINAL_STATES = /* @__PURE__ */ new Set(["completed", "partial", "failed", "cancelled"]);
async function cancelCommand(runId) {
  ...
  await writeFile2(join2(outputDir, "cancel.requested"), (/* @__PURE__ */ new Date()).toISOString() + "\n", "utf8");
...
// runner/src/commands/list.ts
import { readdir } from "node:fs/promises";
import { join as join3 } from "node:path";
async function listCommand() {
  const runsDir = join3(resolveWorkspaceRoot(), ".codex-deep-research", "runs");
  try {
...
async function reportCommand(runId) {
  ...
  const reportPath = join4(runDir, "report.md");
  const summaryPath = join4(runDir, "report.summary.md");
  await access2(reportPath).catch((error) => {
    if (error.code === "ENOENT") {
      throw new Error(`Report file is missing for ${runId}: ${reportPath}`);
    }
    throw error;
  });
  const summary = await readFile3(summaryPath, "utf8").catch((error) => {
    if (error.code === "ENOENT") {
      return void 0;
    }
    throw error;
...
// runner/src/workers/codex-exec-worker.ts
import { spawn } from "node:child_process";
function buildCodexExecSpawnOptions(options) {
...
var CodexExecWorkerClient = class {
  constructor(options) {
...
// runner/src/workflow/schemas.ts
var ScopeSchema = external_exports.object({
  question: external_exports.string().min(1),
  summary: external_exports.string().min(1),
  angles: external_exports.array(
...
var SearchSchema = external_exports.object({
  results: external_exports.array(
    external_exports.object({
      ...
      relevance: external_exports.enum(["high", "medium", "low"])
    })
  ).max(6)
});
var ExtractSchema = external_exports.object({
  sourceQuality: external_exports.enum(["primary", "official", "secondary", "community", "commercial", "forum", "unreliable"]),
  publishDate: external_exports.string().optional(),
  claims: external_exports.array(
    external_exports.object({
      claim: external_exports.string().min(1),
      quote: external_exports.string().min(1),
      importance: external_exports.enum(["central", "supporting", "tangential"])
    })
  ).max(5)
});
var VerdictSchema = external_exports.object({
  refuted: external_exports.boolean(),
  evidence: external_exports.string().min(1),
  confidence: external_exports.enum(["high", "medium", "low"]),
  counterSource: external_exports.string().optional()
});
var ReportSchema = external_exports.object({
  ...
  findings: external_exports.array(
    external_exports.object({
      ...
      evidence: external_exports.string().min(1),
      vote: external_exports.string().optional()
    })
  ).min(1),
  caveats: external_exports.string().min(1),
  openQuestions: external_exports.array(external_exports.string()).default([])
});

// runner/src/workflow/primitives.ts
async function phase(runtime, phaseName) {
...
async function agent(runtime, task, options = {}) {
  ...
  try {
    ...
    try {
      const workerResult = await runtime.worker.run(task);
      result = options.parse ? options.parse(workerResult) : workerResult;
      hasResult = true;
      runtime.status.progress.completed += 1;
      await runtime.store.emit(runtime.runId, { type: "task.completed", taskId: task.label, message: task.label }).catch((error) => {
        emitError = error;
      });
    } catch (error) {
      workerError = error;
    ...
    runtime.status.progress.running -= 1;
    await runtime.store.writeStatus(runtime.status).catch((error) => {
      statusError = error;
    });
  }
  if (workerError !== void 0) {
    throw workerError;
  }
  if (emitError !== void 0) {
    throw emitError;
...
// runner/src/runtime/checkpoint.ts
import { mkdir as mkdir3, writeFile as writeFile3 } from "node:fs/promises";
import { join as join5 } from "node:path";
async function writeCheckpoint(store, runId, name, state) {
  const outputDir = await store.getRunDir(runId);
  const dir = join5(outputDir, "checkpoints");
  await mkdir3(dir, { recursive: true });
  const path = join5(dir, `${name}.json`);
  await writeFile3(path, JSON.stringify(state, null, 2), "utf8");
  return path;
}

// runner/src/report/report-writer.ts
import { writeFile as writeFile4 } from "node:fs/promises";
import { join as join6 } from "node:path";
async function writeReports(input) {
  const reportPath = join6(input.outputDir, "report.md");
  const summaryPath = join6(input.outputDir, "report.summary.md");
  const sourcesPath = join6(input.outputDir, "report.sources.md");
  const findings = input.findings.length > 0 ? input.findings.map((finding, index) => `### ${index + 1}. Finding
...
${sources}
`;
  await writeFile4(reportPath, report, "utf8");
  await writeFile4(summaryPath, input.summary + "\n", "utf8");
  await writeFile4(sourcesPath, sources + "\n", "utf8");
...
// runner/src/workflow/deep-research.workflow.ts
async function stopIfCancellationRequested(runtime) {
  ...
  try {
    ...
    const scope = await agent(
      runtime,
      {
        ...
        prompt: `Decompose this research question into 3-6 complementary search angles.
...
Return structured JSON only.`
    ...
    await input.store.emit(input.manifest.runId, { type: "phase.started", phase: "completed", message: "completed" }).catch(() => void 0);
    await input.store.emit(input.manifest.runId, { type: "report.written", message: reportPath }).catch(() => void 0);
  } catch (error) {
    const currentStatus = await input.store.readStatus(input.manifest.runId).catch(() => void 0);
    if (currentStatus?.state === "cancelled") {
      throw error;
    }
    runtime.status.state = "failed";
    await input.store.writeStatus(runtime.status).catch(() => void 0);
    await input.store.emit(input.manifest.runId, {
      type: "run.failed",
      message: error instanceof Error ? error.message : String(error)
    }).catch(() => void 0);
    throw error;
...
// runner/src/commands/run.ts
async function runCommand(runId) {
  const store = new RunStore(resolveWorkspaceRoot());
...
// runner/src/commands/start.ts
import { spawn as spawn2 } from "node:child_process";
import { existsSync } from "node:fs";
import { dirname as dirname2, join as join7 } from "node:path";
import { fileURLToPath } from "node:url";
var MODES = ["mixed", "web", "repo"];
var DEPTHS = ["quick", "standard", "deep"];
...
function resolveStartLauncher(currentModuleUrl) {
  ...
  if (normalized.endsWith("/dist/commands/start.js")) {
    const cliPath = join7(dirname2(currentFile), "..", "cli.js");
  ...
  if (normalized.endsWith("/runner/src/commands/start.ts")) {
    const pluginRoot = join7(dirname2(currentFile), "..", "..", "..");
    const tsxCliPath = join7(pluginRoot, "node_modules", "tsx", "dist", "cli.mjs");
    const cliPath = join7(pluginRoot, "runner", "src", "cli.ts");
...
async function startCommand(question, rawOptions) {
  ...
  const child = spawn2(launcher.command, launcher.args(manifest.runId), {
    cwd: workspace,
    detached: true,
  ...
  child.unref();
  console.log(JSON.stringify({ runId: manifest.runId, statusPath: join7(manifest.outputDir, "status.json") }, null, 2));
}
function parseChoice(optionName, rawValue, choices) {
...
// runner/src/commands/status.ts
async function statusCommand(runId) {
  const status = await new RunStore(resolveWorkspaceRoot()).readStatus(runId);
  console.log(JSON.stringify(status, null, 2));
}

// runner/src/commands/watch.ts
import { setTimeout as setTimeout2 } from "node:timers/promises";
import { readFile as readFile4 } from "node:fs/promises";
import { join as join8 } from "node:path";
var TERMINAL_STATES2 = /* @__PURE__ */ new Set(["completed", "partial", "failed", "cancelled"]);
async function readEvents(path) {
  return await readFile4(path).catch((error) => {
    if (error.code === "ENOENT") {
...
async function watchCommand(runId, options = {}) {
  ...
  while (true) {
    ...
    if (TERMINAL_STATES2.has(status.state)) {
      await setTimeout2(terminalDrainMs);
...
program2.name("codex-deep-research").description("Run Codex deep research workflows").version("0.1.0");
program2.command("start").argument("<question>").option("--mode <mode>", "mixed|web|repo", "mixed").option("--depth <depth>", "quick|standard|deep", "standard").option("--max-concurrency <n>", "maximum concurrent workers", "8").option("--max-tasks <n>", "maximum logical tasks", "120").option("--debug-prompts", "reserved prompt capture manifest flag", false).action(startCommand);
program2.command("run").argument("<runId>").description("execute a queued research run").action(runCommand);
program2.command("status").argument("<runId>").description("print run status JSON").action(statusCommand);
program2.command("watch").argument("<runId>").description("follow run events until the run reaches a terminal state").action(watchCommand);
program2.command("report").argument("<runId>").description("print report paths and summary").action(reportCommand);
program2.command("cancel").argument("<runId>").description("request cooperative cancellation for an active run").action(cancelCommand);
program2.command("list").description("list known research run ids").action(listCommand);
